...
export class BatchUploadsComponent implements OnInit {
  ...
  tableHeader: string[] = ['ID', 'Description', "Images", "Size", 'Date Created',];
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      let processStatus = params['is_active'] ? params['is_active'] : '';

      this.fetchVirtualMachines(processStatus, searchTerm, cursor);
    });
    this.fetchStatistics();

  }

  fetchVirtualMachines(processStatus: string = '', searchTerm: string, cursor: string) {
    this.datasetService.getVirtualMachines(processStatus, searchTerm, cursor).subscribe({
      next: (res: any) => {
  ...
  fetchStatistics() {
    this.datasetService.getStatistics().subscribe({
      next: (response: any) => {
        this.statistics = response;
      },
      error: (error: any) => {
        console.error(error);
        this.toastr.warning('Something happened!', 'Unable to fetch the statistics at this time');
      }
    });
  }
